...
public class responseWatchlist {
	...
	public static Response getRECOResponse(String URL, String username, String password) {
		...
		String Uri = URL;
		respp = given().headers("X-ACCESS-TOKEN", getXAccessToken(), "Authorization", getBearerToken(username, password)).when().get(Uri);
	...
	public static ArrayList<String> WatchlistValidationEpisodesTab() {
		...
		for(int i=0; i<totalShows;i++) {
			...
			for(int j = 0 ; j < totalEpisodes; j++) {
				EpisodeAssetSubtype.add(respp.jsonPath().getString("show["+i+"].episodes["+j+"].asset_subtype")) ;
...
//		WatchlistValidationMoviesTab();
	...
	public static String getBearerToken(String email,String password)
	{
		...
		req.config(com.jayway.restassured.RestAssured.config().encoderConfig(com.jayway.restassured.config.EncoderConfig.encoderConfig().appendDefaultContentCharsetToContentTypeIfUndefined(false)));
		...
		String Uri = "https://gwapi.zee5.com/content/collection/0-8" +page+ "?page=2&limit=5&item_limit=20&country=IN&translation=en&languages=" + contLang + "&version=6";
//		String Uri = page;
		...
		public static Response getSpecificViewAllResponse(String id,String lang)
		{
			...
			String Uri = "https://gwapi.zee5.com/content/collection/0-8" + id + "?page=1&limit=24&item_limit=20&country=IN&translation=en&languages=en,kn&version=6";
			viewallResp = given().headers("X-ACCESS-TOKEN", getXAccessToken()).when().get(Uri);
